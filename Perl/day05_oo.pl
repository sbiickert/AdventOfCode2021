...
sub parse_input {
	...
	while (<$input>) {
		$_ =~ m/^(\d+),(\d+)\D+(\d+),(\d+)$/;
		my %vent_line;
		my $start = Point2D->new('px' => $1+0, 'py' => $2+0); #numify
		my $end = 	Point2D->new('px' => $3+0, 'py' => $4+0); #numify
		$vent_line{'line'} = Line2D->new('from' => $start, 'to' => $end);
		$vent_line{'horizontal'} = $start->py == $end->py; # no change in y
		$vent_line{'vertical'} = $start->px == $end->px;   # no change in x
		$vent_line{'points'} = interpolate($vent_line{'line'});
...
sub interpolate {
	...
	my $start = dclone($line->from);
	#print Dumper($line, $start);
	push(@points, $start);
	
	my $prev = $start;
	for my $i (1..$length) {
		my $pt = Point2D->new('px' => $prev->px + $stepx, 'py' => $prev->py + $stepy);
		push(@points, $pt);
		$prev = $pt;
	}
